...
## 工程化整理
### 输入异常与边界情况
——比如用户输入不存在的学生、数组为空时删除/查找、输入成绩不合法等。
...
#### 边界情况：删除学生以后，要不要 realloc 缩小内存？
realloc 不仅能扩大内存，也可以缩小已经分配的区域；成功时返回新的指针，失败时原来的内存仍然有效，

所以仍然应该使用临时指针接收返回值。

你会频繁地重新调整内存。

而且 realloc 有可能移动整个内存块。

所以实际程序中经常会把：“有效元素数量”和“已经申请的容量”分开管理。
```text
例如：
size     = 3   ← 实际有几个学生
capacity = 10  ← 当前最多能放几个学生
...
#### 3.既然删除后留下的内存还可以使用，添加时就应该直接利用它，而不是每次都重新调整内存大小。
size     = 当前有几个学生

capacity = 当前内存最多能放几个学生
